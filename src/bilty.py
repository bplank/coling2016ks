...
def main():
    ...
    parser.add_argument("--pred_layer", nargs='*', help="layer of predictons for each task") # for each task the layer on which it is predicted (default 1)
    ...
    parser.add_argument("--test", nargs='*', help="test folder task 1", required=False) # should be in the same order/task than train, thus need to put the task we want to evaluate on at the begininning of the train list...
    parser.add_argument("--dev", help="dev", required=False) 
...
class NNTagger(object):
    ...
    def fit(self, list_folders_name, num_iterations, train_algo, dev=None):
        ...
        loss_per_task = defaultdict(float)
        total_tagged_per_task = defaultdict(int)
        ...
        for iter in range(num_iterations):
            ...
            for ((word_indices,char_indices),y, task_of_instance) in train_data:
                ...
                loss_per_task[task_of_instance] += lv
                total_tagged_per_task[task_of_instance] += len(word_indices)

                loss1.backward()
                trainer.update()

            print("iter {2} {0:>12}: {1:.2f}".format("total loss",total_loss/total_tagged,iter), file=sys.stderr)
            for task_id in loss_per_task.keys():
                print("iter {2} task_id {3} {0:>12}: {1:.2f}".format("total loss",loss_per_task[task_id]/total_tagged_per_task[task_id],iter, task_id), file=sys.stderr)
    ...
    def build_computation_graph(self, num_words, num_chars):
        ...
        for layer_num in range(0,self.h_layers):
            print(">>>", layer_num, "layer_num", file=sys.stderr) 
